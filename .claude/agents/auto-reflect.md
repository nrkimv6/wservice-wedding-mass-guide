...
- 없으면 같은 파일명을 `.worktrees/plans/docs/archive/`, `common/docs/archive/`, `docs/archive/` 순서로 탐색해 대체한다.
...
- plan 생성 전 `_path-rules.md` 동적 폴백 로직으로 plan 루트를 결정한다.
  - wtools 공통: `.worktrees/plans/docs/plan/` 우선, `common/docs/plan/` fallback
  - 일반 프로젝트: `docs/plan` (폴백)
