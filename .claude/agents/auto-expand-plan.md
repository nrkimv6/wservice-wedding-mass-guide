...
4. Python/백엔드 수정 시 T1~T5 테스트 Phase 체크박스 생성:
   ...
   - T4: E2E 테스트 — **반드시 Glob으로 `tests/**/*e2e*`, `tests/**/*integration*` 파일 탐색 후 결정**. 1개라도 존재하면 포함 필수. "CLI 도구라서", "프레임워크 없어서" 같은 인상 기반 스킵 절대 금지. Phase 생략 금지, 스킵 시 사유 체크박스 필수
   - T5: HTTP 통합 테스트 — **반드시 Glob으로 `tests/**/*http*`, `tests/**/*api*` 파일 탐색 후 결정**. HTTP 서버가 아닌 것이 코드로 확인된 경우에만 스킵. Phase 생략 금지, 스킵 시 사유 체크박스 필수
