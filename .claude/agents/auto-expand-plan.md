...
4. Python/백엔드 수정 시 T1~T5 테스트 Phase 체크박스 생성:
   ...
   - T4: E2E 테스트 (프로젝트에 E2E 존재 시 항상 포함. Phase 생략 금지, 스킵 시 사유 체크박스 필수)
   - T5: HTTP 통합 테스트 (프로젝트에 HTTP 테스트 존재 시 항상 포함. Phase 생략 금지, 스킵 시 사유 체크박스 필수)
   - **T4/T5 금지 사유**: "단위 테스트로 커버됨", "수동 테스트", "실제 환경 필요" — 이런 이유로 스킵 불가
5. 상태를 `검토완료`로 Edit
