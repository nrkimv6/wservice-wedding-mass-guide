...
`/reflect`는 3가지 모드로 동작한다. 입력 형태로 자동 판별한다.

| 모드 | 트리거 | 입력 범위 | 용도 |
|------|--------|----------|------|
| **자동(aggregator)** | executor가 내부 호출 (`source=executor`) | `docs/session-memo/{run_id}_*.md` | plan-runner 파이프라인 종료 후 분산 mini-reflect 교차 분석 |
| **수동(aggregator)** | `/reflect {run_id}` — run_id 명시 | 해당 run_id의 session-memo | 특정 파이프라인 회고 재실행 |
| **기존 대화형** `[deprecated]` | `/reflect` (인자 없음) 또는 `/reflect 조사` | 현재 대화 컨텍스트 | 레거시 즉시 회고 |

### 모드별 분기

1. **자동(aggregator)**: executor가 `source=executor` + `run_id=...`로 호출. `app/modules/dev_runner/services/reflect_aggregator.py::aggregate(run_id)` 경로를 그대로 인라인 수행. 결과: `docs/reflect/{run_id}_final.md`.
2. **수동(aggregator)**: 사용자가 `/reflect 2026-04-08_abc123` 형태로 호출. 자동 모드와 동일한 aggregator 경로를 재실행. 기존 `final.md`는 `.prev.md`로 백업 후 덮어쓰기.
3. **기존 대화형** `[deprecated]`: run_id 없이 호출 시 기본 모드. **반드시 stderr에 `[deprecated] /reflect 기본 대화형 모드는 1~2 릴리스 후 aggregator로 전환됩니다. 명시적으로 /reflect {run_id}를 사용하거나, plan-runner 파이프라인을 통해 자동 호출되도록 권장합니다.` 경고를 출력**한 뒤 기존 1~3단계(4가지 추출 → 계획서 생성 → review-plan)를 수행한다.

### 하위 호환 원칙

- **하드 교체 금지**: 기존 대화형 모드는 1~2 릴리스 동안 **유지**. 기본값(run_id 없이 호출) = 기존 대화형 모드.
- 1~2 릴리스 후에만 기본값을 aggregator 권장으로 전환 검토.
- 기존 `monitor-page`, `wtools` 등의 수동 `/reflect` 호출은 본 변경으로 깨지지 않는다.
...
- **plan-runner 경로** — 수동 `/reflect`는 대화 컨텍스트 기반(기본값, `[deprecated]` 경고 출력), 자동 파이프라인은 `auto-reflect` 에이전트(로그+실패표 기반)를 사용. aggregator 모드는 `/reflect {run_id}` 또는 executor 내부 호출로 활성화
- **모드 판별 책임** — "입력" 섹션의 3모드 표를 따라 run_id 유무로 판별. 모호할 경우 기존 대화형으로 fallback + `[deprecated]` 경고
