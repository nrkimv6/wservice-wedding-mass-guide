...
**2.7 owner set 역할 판정 (attach 모드 크로스 플랜 갱신):**
...
6. **LLM Wiki ingest** — archive 이동 + 헤더 추가 성공 후 반드시 호출
   ...
   - 이후 `/archive-sweep` 스킬의 "LLM Wiki ingest" 절차(DB ingest API 호출 + 매칭 가이드 AUTO 블록 1행 insert + `_meta.yaml` `last_archive_scan` 갱신)에 위임
   - ingest 실패는 Write-Error + 복구 힌트 출력, done 전체 흐름은 중단하지 않음
...
> **태그 직접 감지 (키워드 우선순위 동등)**: 항목 텍스트에 이미 `(→ MANUAL_TASKS)` 태그가
> 포함된 경우 키워드 검사 없이 수동 항목으로 즉시 판정한다.
> 이는 /implement 스킬이 구현 중 수동 태그를 먼저 부여한 경우의 중복 감지를 방지한다.
