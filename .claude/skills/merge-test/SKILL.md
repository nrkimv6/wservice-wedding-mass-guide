...
$serviceLockTargets = $branchFiles | Where-Object { $sensitiveMergeFiles -contains $_ }
$runningServices = @(
...
- `$serviceLockTargets`가 1개 이상이고 `$runningServices`도 있으면 즉시 중단:
  ...
  - merge 대상 `service_run.py` 경로와 실행 중인 서비스명을 함께 출력
...
4. **T4 실행** (E2E 존재 시):
   ...
   ```
   - `tests/e2e/` 또는 명시적 테스트 파일 경로가 들어간 명령에서 `0 selected`, `N deselected / 0 selected`가 나오면 `MERGE_TEST_FAILED[selection_contract]`로 즉시 중단한다. `PASS`, `해당 없음`, `SKIP(no-match)`로 기록하지 않는다.
