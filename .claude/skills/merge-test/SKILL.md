...
- unmerged marker가 없고 `MERGE_HEAD`만 남아 있으면 commit-ready pending merge로 분류한다. 현재 owner의 merge임을 확인할 수 있으면 직접 `git commit`을 호출하지 말고 `D:\work\project\tools\common\commit.ps1`로 merge commit을 마무리한 뒤 1단계로 계속한다.
...
if (Test-Path $mergeHead) {
  ...
  Write-Warning "If this merge belongs to the current owner, finish it via D:\work\project\tools\common\commit.ps1. If owner is unclear, wait/report instead of final abort."
}
