...
### 1.7단계: T4/T5 스킵 Glob 재검증

plan에 T4/T5가 `[x] 스킵`으로 체크되어 있으면, merge-test가 독립적으로 Glob을 실행하여 스킵 판정을 재검증한다.
expand-todo의 판단을 신뢰하지 않고, 직접 확인한다.

**검증 절차:**
1. plan에서 T4/T5 `[x] 스킵` 항목 탐지 (없으면 이 단계 스킵)
2. T4 스킵인 경우: `Glob tests/**/*http*`, `Glob tests/**/*api*` 실행
   T3 스킵인 경우: `Glob tests/**/*e2e*`, `Glob tests/**/*integration*` 실행
3. 판정:
   - Glob 결과 1개 이상 존재 → **스킵 거부, TC 자동 작성 후 실행**
   - Glob 결과 0개 → 스킵 허용, 정상 통과

**스킵 거부 시 자동 복구:**
1. plan의 `[x] 스킵` 체크박스를 `[ ]`로 되돌린다
