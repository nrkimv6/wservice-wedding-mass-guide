...
2. **API 헬스체크 폴링** (최대 2분, 5초 간격):
   ...
   for ($i = 1; $i -le 24; $i++) {
     try {
       ...
       if ($i -eq 24) {
         Write-Host "⚠️ 2분 초과 — 실서버 미응답 상태로 계속 진행"
       }
     }
   }
   ```
...
4. **T4 실행** (E2E 존재 시):
   ```powershell
   pytest tests/e2e/ -m e2e  # tests/e2e/ 하위만 실행 (dev_runner e2e 마커와 분리)
   # T4 기준: 실서버(localhost:8001) 또는 실브라우저(Playwright) 필요 — TestClient 기반은 T3(integration)
   ```
