...
6. **T5 실행** (HTTP 통합):
   ...
   **dev-runner smoke residual 계약**:
   ...
   - wtools 원본 skill/agent 계약 변경이 monitor-page mirror에 필요하면 downstream sync evidence가 있어야 한다. sync evidence가 없으면 monitor-page T5는 실행하지 않고 `DOWNSTREAM_SYNC_EVIDENCE_MISSING`으로 중단한다.
