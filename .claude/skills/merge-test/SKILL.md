...
- commit-ready pending merge라도 staged 구현성 파일이 `.agents/`, `.claude/`, `.gemini/`, `app/`, `frontend/`, `scripts/`, `tests/`에 있으면 `commit.ps1` guard를 우회하지 않는다. 특히 child repo mirror sync merge는 root에서 닫지 않는다. `ROOT_GUARD_BLOCKED_PENDING_SYNC_MERGE` evidence를 남기고 보존/abort 후 upstream sync 재생성 또는 remote fast-forward 수신 evidence로만 전환한다.
...
- `MERGE_HEAD` 존재 자체를 final abort 사유로 닫지 않는다. 먼저 helper preflight JSON과 `git status --porcelain=v1`로 상태를 분류한다.
...
6. **T5 실행** (HTTP 통합):
   ...
   **dev-runner smoke residual 계약**:
   ...
   - wtools 원본 skill/agent 계약 변경이 monitor-page mirror에 필요하면 downstream sync evidence가 있어야 한다. sync evidence가 없으면 monitor-page T5는 실행하지 않고 `DOWNSTREAM_SYNC_EVIDENCE_MISSING`으로 중단한다. 이 문장은 mirror 직접 edit/commit 허가가 아니라 sync/read-back gate다.
