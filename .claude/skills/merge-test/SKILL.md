...
Preflight and cleanup evidence must come from helper CLI contracts before any merge mutation when `common\tools` exists. Use `common\tools\merge-test-preflight.ps1 -PlanFile <plan> -RepoRoot <repo> -Json` for pending merge, branch/worktree, dirty, and service-lock evidence. Use `common\tools\merge-test-cleanup.ps1 -PlanFile <plan> -RepoRoot <repo> -Json` for post-merge cleanup evidence. If a downstream repo has no `common\tools` helper surface, record `helper_unavailable` and use the direct read-back checklist instead. AI owns the merge/abort decision.
...
- repo에 `common\tools`가 있는데 guard entrypoint가 없거나 session 활성화/상태 확인이 실패하면 `GIT_GUARD_NOT_ACTIVE`로 중단한다.
- repo에 `common\tools`가 없으면 guard hard stop이 아니라 `helper_unavailable` fallback으로 전환한다.
...
- helper가 있으면 helper-first다. `merge-test-preflight.ps1`, `merge-test-cleanup.ps1`, `enable-git-guard.ps1` evidence를 우선 사용한다.
