...
**경로 규칙**: CLAUDE.md `문서 위치 규칙` 테이블을 참조하라. 실제 경로 선택은 [`_path-rules.md`](./_path-rules.md)의 helper 우선순위(`PLAN_ROOT` → `.worktrees/plans/docs/plan` → `docs/plan`)를 먼저 따른다.

**wtools 감지**: 현재 디렉토리에 `common/tools/` 폴더 존재 여부로 판단
- **있으면**: wtools 내부 → CLAUDE.md의 plan 경로에 공통 계획 저장 + `.worktrees/plans/TODO.md` ledger 등록 **실행**
- **없으면**: 외부 프로젝트 → CLAUDE.md의 plan 경로에 저장 + 해당 프로젝트의 `_path-rules.md`/plans ledger 정책을 따른다

task ledger 판단도 `_path-rules.md` helper의 docs commit root를 따른다. wtools에서는 `.worktrees/plans/TODO.md`와 `.worktrees/plans/docs/DONE.md`가 canonical이며, repo root `TODO.md`, `docs/DONE.md`, `wtools/TODO.md`는 직접 갱신하지 않는다.
...
상담성 발화에서 나온 제안은 plan requirement가 아니라 option/proposal로 기록한다. 예: "Tracking에 추가하는 건 어떤가" 같은 문장은 사용자가 `적용해`, `구현해`, `요구사항으로 넣어`처럼 명시하기 전까지 실행 TODO가 아니며, corrective action plan에는 `사용자 승인 범위`와 `금지된 확대 해석`을 분리해 적는다.

**상담성 제안 기록 규칙:**
- `Tracking에 추가`, `TrackingItem 정리`, `삭제/되돌리기/추가도 가능`처럼 사용자가 가능성을 묻거나 제안한 문장은 `검토 옵션/제안 (미승인)` 섹션에만 기록한다.
- 미승인 제안은 `요구사항`, `실행 TODO`, `완료 조건`, `수정 범위`, `승인된 작업`에 넣지 않는다.
...
1. **`.worktrees/plans/TODO.md`를 Read로 연다**
...
6. repo root `TODO.md`, `docs/DONE.md`, `wtools/TODO.md`는 legacy/stub 또는 downstream mirror read-back 대상이므로 이 단계에서 쓰지 않는다.
...
3. **V2-S. 보안 패턴 전파**: CLAUDE.md에 보안 패턴 레지스트리가 있으면, plan이 새로 추가/수정하는 코드에 해당 패턴이 적용되어야 하는지 검증. 예: 새 git subprocess 호출 추가 시 safe.directory 패턴 적용 여부 확인.

**[기각 대안 잔여 위험 검증]** 대안을 기각할 때, 기각된 대안이 해결하려던 문제가 채택된 대안으로 **완전히** 해소되는지 검증해야 한다. '범위가 넓다'는 기각 사유가 될 수 없다 — 넓더라도 필요하면 적용해야 한다.
...
1. **plan 파일 존재** — `_path-rules.md` 동적 폴백으로 확인한 plan 루트에 대표 문서 존재
...
4. **plans/TODO.md** — Pending에 plan 링크 항목 존재 + 날짜 오늘
5. **root legacy ledger 미변경** — repo root `TODO.md`, `docs/DONE.md`, `wtools/TODO.md`에 신규 task ledger 항목을 쓰지 않음
...
**스테이징 대상 (변경된 파일만):**
- `Resolve-DocsCommitRoot` 기준 plan/archive/history 경로의 `*.md` + exact file `TODO.md`, `docs/DONE.md`

**커밋 실행 절차 (반드시 이 순서):**
1. `git status --porcelain` — 변경 파일 목록 확인
2. 화이트리스트 파일만 **개별** `git add` — 파일명 하나씩 명시:
   - `git add "docs/plan/YYYY-MM-DD_foo.md"`
   - `git add "TODO.md"` (plans commit root 기준 `.worktrees/plans/TODO.md`)
...
5. 커밋 스크립트 실행: `docs: plan {주제}`
   - ⚠️ plans lineage 모드에선 `commit.ps1` 실행 후 push까지 1세트다. push 대상은 literal `origin plans`가 아니라 현재 docs commit root가 추적하는 upstream이며, root `main`/일반 feature branch에서는 continue하지 않는다.
6. 커밋 성공 여부 확인: 실패 시 원인 보고 후 재시도하고, 성공 전까지 6단계로 진행하지 않는다.

## 🔴 자동 커밋 안전 규칙

git add 허용 경로 (화이트리스트):
- `Resolve-DocsCommitRoot` 기준 `docs/plan/**/*.md`
- `Resolve-DocsCommitRoot` 기준 `docs/archive/**/*.md`
- `Resolve-DocsCommitRoot` 기준 `docs/history/**/*.md`
- `Resolve-DocsCommitRoot` 기준 `TODO.md`, `docs/DONE.md`
- wtools plans 워크트리에서는 위 `TODO.md`, `docs/DONE.md`가 각각 `.worktrees/plans/TODO.md`, `.worktrees/plans/docs/DONE.md`를 뜻한다
...
4. plans 워크트리 dirty 감지 시, 이번 실행이 수정한 파일만 `Resolve-DocsCommitCandidates` 기준으로 분리해서 커밋
...
- `git add "TODO.md"` (plans commit root 기준)
...
**요약 필드 규칙:**
- 헤더 블록쿼트에 `> 요약:` 필드를 포함한다
- 형식: `{증상/불편함} — {핵심 목적/변경}`
- `fix:` 유형은 증상 **필수**, `feat:`/`refactor:` 유형은 **선택** (불편함이 없으면 목적만)
- plan-list 조회 시 테이블에 표시되어 유사 증상 plan 묶기에 활용

| 유형 | 예시 |
|------|------|
| `fix:` | `plan-list 요약이 목적 위주라 증상 기반 검색이 어렵다 — 요약 필드를 증상 우선으로 개선` |
| `feat:` | `예약 목록에 날짜 필터 추가 — 조회 범위를 날짜로 좁히는 기능 구현` |
| `refactor:` | `merge_stage.py가 너무 커서 책임 파악이 어렵다 — 책임 단위로 분리` |
