...
**Phase T3: E2E 테스트 수행 및 수정**
- **포함 여부 결정 전 필수**: Glob으로 `tests/**/*e2e*`, `tests/**/*integration*` 탐색 → 1개라도 있으면 포함
- 탐색 결과 파일 없음이 확인된 경우에만 스킵 허용 (스킵 시 Glob 탐색 근거 체크박스 필수)
- 시나리오별 개별 체크박스
- **Phase 자체 생략 금지** — 스킵하더라도 Phase 헤더 + 스킵 사유 체크박스 필수

**Phase T4: HTTP 통합 테스트**
- **포함 여부 결정 전 필수**: Glob으로 `tests/**/*http*`, `tests/**/*api*` 탐색 → **프로젝트 전체에서 1개라도 있으면 → 기능 전용 파일 유무와 무관하게 해당 변경에 대한 T4 TC를 신규 작성한다**
- HTTP 서버가 아닌 것이 코드로 확인된 경우에만 스킵 허용 (스킵 시 확인 근거 체크박스 필수)
- `{METHOD} {endpoint}` 정상/에러 응답별 개별 체크박스
- **Phase 자체 생략 금지** — 스킵하더라도 Phase 헤더 + 스킵 사유 체크박스 필수

**T3/T4 금지 사유 (이런 이유로 스킵하면 안 됨):**
- "단위 테스트로 커버됨" — T3/T4는 단위 테스트와 검증 범위가 다름
- "수동 테스트" — T3/T4는 pytest로 자동 실행하는 테스트
...
**🔴 탐색 없이 스킵 결정 = 규칙 위반**: T3/T4 스킵 전 Glob 탐색을 반드시 실행하고 결과를 근거로 제시할 것
...
4. 기존 plan의 스킵 사유가 금지 사유(3단계 T3/T4 금지 사유 목록)에 해당하는지 확인
