...
2. **TODO Pending + Plan 문서** - 동등한 우선순위로 모두 스캔
   ...
   - CLAUDE.md 문서 위치 규칙의 plan 경로 미완료 항목
...
3. **개선 아이디어** - CLAUDE.md 문서 위치 규칙의 improvement 경로 (P0 → P1 순)
...
**개선 아이디어 확인:**
- CLAUDE.md 문서 위치 규칙의 improvement 경로 스캔
- `완료` 표시 없는 P0 항목 찾기
- 없으면 P1 항목 찾기
...
2. **파일 수정**: 선택한 항목의 `[ ]` → `[→$workerId]`로 변경
   - plan 파일: CLAUDE.md 문서 위치 규칙의 plan 경로
...
다음 중 하나를 해보세요:
1. CLAUDE.md 문서 위치 규칙의 plan 경로에 새 계획 문서 작성
2. `.worktrees/plans/TODO.md`에 작업 추가
3. CLAUDE.md 문서 위치 규칙의 improvement 경로 검토
```
...
4. **이전 날짜면** → CLAUDE.md 문서 위치 규칙의 plan 경로를 스캔하여 `.worktrees/plans/TODO.md` 갱신 후 스캔
