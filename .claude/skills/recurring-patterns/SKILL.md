...
## Parser And Advisory Contract

정규식과 키워드는 아래 네 가지로 분류한다.

| 분류 | 자동화 가능 범위 | 예시 | 금지 |
|---|---|---|---|
| deterministic parser | 구조화된 입력을 읽고 같은 출력으로 판정한다 | checkbox marker, `> branch:`, `MERGE_HEAD`, unmerged marker, `git status --porcelain` | 자연어 의도 추론 |
| advisory trigger | AI 판단에 넘길 evidence만 만든다 | `migration`, `placeholder`, `workaround`, `삭제`, `fix:` 같은 자연어 seed | 단독 Phase 삽입, 단독 차단, 문구 자동 보정 |
| AI-required judgment | 주변 문맥, 실제 파일, 실행 근거를 함께 판단한다 | DB 변경 여부, 환경오염 여부, 재발 경로 방어 여부 | helper 결과만으로 mutation-ready 선언 |
| forbidden auto-mutation | 키워드만으로 코드를 바꾸거나 plan을 완료 처리하지 않는다 | keyword-only DB-Direct, placeholder-only review-plan 차단, filename-only Phase 보정 | 자동 수정, 자동 체크, 자동 archive |

advisory trigger는 `category`, `pattern`, `path`, `line`, `context`, `confidence`, `required_human_or_ai_judgment` 같은 evidence를 남기는 데까지만 사용한다. 자동 수정이 필요하면 deterministic structural evidence와 AI 판단이 모두 있어야 한다.
...
| # | 패턴 | 위치 | 핵심 규칙 |
|---|------|------|----------|
| # | 패턴 | 위치 | 핵심 규칙 | 대표 적용 파일 |
|---|------|------|----------|---------------|
| 0 | Svelte 5 문법 강제 | 프론트 | `on:click` 금지, `onclick` 필수. Svelte 4 문법 전면 금지 | `frontend/src/routes/+layout.svelte` |
| 1 | `createSelection()` | 프론트 | 체크박스 목록 = Set 기반 유틸 필수 | `frontend/src/lib/utils/selection.svelte.ts` |
| 2 | `toast` | 프론트 | `alert()` / 자체 구현 금지, toast만 사용 → [user-feedback.md](user-feedback.md) | `frontend/src/lib/stores/toast.ts` |
| 3 | 로컬 상태 업데이트 | 프론트 | 액션 후 전체 reload 대신 로컬 갱신 | `frontend/src/routes/instagram/+page.svelte` |
| 4 | 페이지네이션 유틸 | 프론트 | offset/page 상태 직접 선언 금지 → [pagination.md](pagination.md) | `frontend/src/lib/utils/pagination.svelte.ts` |
| 5 | 401 콜백 | 프론트 | `location.reload()` 금지, 쿨다운 가드 | `frontend/src/lib/utils/fetch.ts` |
| 6 | 202 + 폴링 | 백엔드 | 5초+ 작업은 비동기 큐 패턴 | `app/modules/google_search/services/search_service.py` |
| 7 | Redis 큐 위임 | 백엔드 | Session 0에서 subprocess 금지 | `app/worker/orchestrator.py` |
| 8 | `_safe_execute` | 백엔드 | 워커 작업별 예외 격리 필수 | `app/worker/base_worker.py` |
