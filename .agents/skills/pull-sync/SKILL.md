...
**ff-only 실패 시 복구 절차**:
1. local main이 origin/main보다 ahead라서 ff-only가 실패하면 root에서 merge/resolve하지 않는다.
2. 현재 local main이 원격에 반영돼야 하는 상태면 사용자가 `git push origin main`으로 origin을 먼저 정렬한 뒤 `git pull --ff-only`를 retry한다.
3. push가 거절되거나 권한이 없으면 sync worker handoff 또는 upstream sync 재생성 evidence를 요구하고, root receiver에서는 `NON_FF_SYNC_BLOCKED`로 중단한다.
4. 이미 remote sync commit이 생성돼 있으면 receiver는 fetch 후 fast-forward 가능 여부만 확인하고, conflict resolution commit을 만들지 않는다.
