...
| 2 | `remaining executable leaf` > 0 | **continue** | 같은 턴에서 다음 leaf 또는 reflect 분석 단위 |
| 3 | `remaining targets` > 0 | **continue** | 같은 턴에서 다음 target |
| 4 | `next owner step` 있음 | **continue** | 같은 턴에서 해당 owner skill 또는 review-plan 단계 실행 |
| 5 | `archive/read-back` 미완료 | **continue** | read-back 또는 owner handoff 완료까지 진행 |
| 6 | 위 조건 모두 해당 없음 | **final** | 완료 보고 허용 |

입력 정의는 현재 reflect 대상과 session targets ledger의 read-back 값을 기준으로 한다. 판정 우선순위는 위 표 순서를 따른다. 중간 결과표 출력, 계획서 1개 생성, 커밋 성공은 판정 입력이 아니라 진행 업데이트다. hard blocker에는 입력 누락, git 실패, 파일 접근 불가, 필수 evidence 누락을 포함한다. 이 결정표는 session targets, 사용자 escalation evidence, review-plan handoff 전부에 적용한다.
...
- 세션의 `declared/processed/excluded/remaining` count를 reflect 입력 요약에 남긴다.
- target count mismatch가 있었던 세션은 Q5 후보로 반드시 검토한다.
- `명시 제외`와 `agent 임의 제외`를 구분하지 못했거나 `excluded_unconfirmed` 상태를 전체 완료에 포함했다면 Q6 교정 후보로 남긴다.
...
- 구현중 사용자 교정 발화는 마지막 technical miss 하나로 축소하지 않고 시간순 ledger로 수집한다.
- 현재 세션에서 기존 active/archive plan에 귀속하더라도 intervention ledger row는 유지한다.
- 후보 분류는 `review-plan 선행`, `target scope 제한`, `완료 여부 재질문`, `dirty/conflict/untracked 조사`, `sync trigger 미실행`, `done/reflect 재호출`을 별도 행으로 나눠 검토한다.
- Q5/Q6 성격 개입이 2개 이상이면 신규 plan이 0개여도 `reflect findings: N건, 신규 plan 0건`으로 보고한다.

| 발화 요지 | 누락/오판 | Q분류 | 처리(plan/new, attach_existing, report-only) | 근거 |
|-----------|-----------|-------|----------------------------------------------|------|
| `{사용자 교정 요약}` | `{누락 또는 오판}` | `{Q5|Q6|Q5/Q6}` | `{plan/new|attach_existing|report-only}` | `{로그/파일/명령 근거}` |

→ 상단 STOP/CONTINUE Decision Table 우선. 이 섹션은 세부 근거 참조용.

### 사용자 escalation evidence gate

- 사용자 escalation은 안전 훈계 대상이 아니라 작업 품질 신호다. 강한 표현을 평가하거나 교정하지 말고, 어떤 작업 누락/오판/완료 축소를 드러냈는지 기록한다.
- escalation evidence 예시는 `왜 멈췄냐`, `다 하지도 않았는데`, `계획서를 다시 읽고`, `남은 작업 계속해`, `stash 했으면 pop 하라고`, `죽을래?`처럼 명시 재지시, 질책, 강한 불만 신호가 포함된 발화다.
- escalation evidence가 있으면 Q5와 Q6 모두를 반드시 검토한다. Q5/Q6를 `해당 없음`으로 닫으려면 발화별로 `report-only`, `attach_existing`, `plan/new` 중 하나와 근거를 ledger에 남겨야 한다.
- 이전 응답이 완료/마무리/추가 작업 없음처럼 닫았는데 사용자가 남은 작업을 다시 지시한 경우, `완료 축소 보고`와 `remaining targets 재확인 누락`을 Q5/Q6 후보로 남긴다.
- escalation evidence가 하나라도 있으면 0건 처리(`리뷰 완료 — 추가 작업 없음`, `리뷰 완료.`, `추가 작업 없음`)는 금지한다. 먼저 `사용자 escalation 처리` 행에 무엇을 다시 확인했고 무엇을 고쳤는지 또는 왜 report-only인지 적는다.
- escalation evidence가 있고 `report-only`로 닫으려 할 때는 해당 행동 유형의 재발 방지 조항이 관련 SKILL.md에 존재하는지 먼저 확인한다. 없으면 `report-only` 금지, `must_plan` 강제다.
- compaction 후 재개 지연으로 escalation이 발생한 경우, 관련 SKILL.md에 `compaction resume gate` 조항이 없으면 `must_plan` 강제다.
...
- **UNTRACKED_ORIGIN_BLOB_RESIDUE**: current HEAD에는 없고 upstream에는 tracked이며 local untracked blob hash가 upstream blob hash와 같은 파일. 발견 시 Q5 지시불이행 또는 Q6 agent 교정 finding으로 승격한다. active plan이 같은 residue gate를 다루면 `attach_existing`, 없으면 `plan/new`로 처리한다. owner plan이 archive라면 새 corrective plan에 archived owner와 dirty-left closeout owner를 모두 남긴다.
- **preexisting staged / expected staged set / staged ownership**: reflect fallback commit 또는 review-plan fallback 중 `git diff --cached --name-status`가 current owner expected staged set과 다르면 Q5/Q6 후보로 승격한다. `commit.ps1 -Files` warning은 success가 아니라 staged ownership 재검증 trigger다.
...
- `T4_T5_CONTRACT_VIOLATION`: T4/T5 체크박스가 `[x]`인데 근거 파일에 `pytest.mark.http_live`가 없고 `page.route("**/*")`/`page.route('**/*')` mock-only 또는 `TestClient` 단독 근거만 있으면 Q4 hard gate finding으로 기록한다. 이 경우 해당 없음 판정 금지, `must_plan` 강제, live T4/T5 follow-up 또는 T3 재분류 plan을 생성/attach한다.

### 1-1단계: 상태 머신 6축 gate (Q4/Q6)

plan 본문, diff, 로그, 또는 의심 코드에 아래 detector seed가 있으면 상태 머신 변경으로 의심하고 Q4/Q6 closeout 전에 6축 누락 여부를 표로 enumerate한다. seed가 0개인 CRUD/문서 plan에는 이 gate를 강제하지 않는다.

- 상태 어휘 seed: `state`, `status`, `phase`, `transition`, `lifecycle`, `terminal`, `waiting`, `pending`, `approval_required`, `완료`, `보류`, `대기`
- writer/race seed: `overwrite`, `late writer`, `race`, `cleanup`, `retry`, `fallback`, `concurrent`, `순서 역전`, `덮어씀`, `stale`
...
- "관련 TC 존재", "상태 전이 테스트 있음", vocabulary grep 통과만으로는 Q4/Q6를 닫지 않는다.
- 6축 중 해당 없는 축은 축 이름과 함께 `N/A: 사유`를 명시해야 한다. 예: `override: N/A: 사용자 override 경로가 없는 내부 상태`.
- detector seed가 있는데 6축 중 evidence 또는 `N/A: 사유`가 빠진 축이 있으면 Q4 `미발견 오류` 또는 Q6 `agent 교정작업 제안` 후보로 남기고, `해당 없음`/`report-only`로 축소하지 않는다.
...
**pre-existing finding promotion guard:** Q1~Q6 발견에서 `pre-existing`, `이번 세션에서 도입되지 않음`, `수정 순증이 작음`은 plan 생성 제외 사유가 아니다. `pre-existing`은 발견 설명, 우선순위, owner 경계에만 기록하고 `계획서: -`의 근거로 쓰지 않는다. 발견 항목이 active plan에 있으면 `existing`, active plan이 없으면 `plan/new` 또는 `user-review`로 남긴다.

Q3에서 과도하게 비대한 파일, 복잡한 조건/분기, 중복, 임시 해결책이 발견되면 기존 active plan 검색을 먼저 수행한다. active plan이 없고 대상 파일/모듈이 식별되면 리팩토링 follow-up plan을 생성한다. active plan도 없고 owner가 불명확한 경우만 `user-review`로 보류하며, 단순 `report-only`로 축소하지 않는다.
...
**product-surface evidence scope guard:** `completion-scope: product_surface` plan 또는 제품 UI/API/runtime surface를 완료했다고 주장한 plan은 `scripts/scratch/`, `scratch/`, `tmp/`, `private/`, `.private/` 같은 scratch/private utility evidence만으로 완료 처리하지 않는다. 제품 surface path/read-back(`app/`, `frontend/`, `backend/`, `src/`, `packages/`, `services/`, `common/tools/` 등)이 없으면 `non_product_only`로 분류하고 target-local `blocked` 또는 Q4 finding으로 남긴다. 이 경우 `계획서: -`, `report-only`, `already complete`로 축소하지 않는다.
...
- 사용자 재지시/질책/강한 불만 신호(`왜 멈췄냐`, `다 하지도 않았는데`, `계획서를 다시 읽고`, `남은 작업 계속해`, `stash 했으면 pop 하라고`, `죽을래?` 등)가 발생했는데도 해당 발화가 드러낸 누락 작업, 완료 축소, read-back 누락을 반영하지 않은 경우
- 사용자가 `[$skill](...SKILL.md)` 링크나 exact skill name을 다시 제시했는데도 실행 대신 설명을 반복한 경우
- 명시 다중 skill 호출에서 일부 skill만 수행하고 `skill | 실행 여부 | evidence | 남은 조치` 결과표를 남기지 않은 경우
- "파일 닫어", "파일 닫기", "닫어", "닫기", "마무리" 지시의 closeout owner routing을 같은 턴에서 수행하지 않고 사용자 재지시 이후에야 `branch/worktree present -> /merge-test; absent -> /done` 판정을 수행한 경우
...
- archived plan으로 완료된 계약이 동일하게 재발하면 `existing` 대신 `recurrence_after_archive`로 분류하고, 같은 범위의 active plan이 없으면 `plan/new`로 승격한다.
- 사용자가 자동 파이프라인을 수동으로 다시 돌려야 했다는 escalation(예: `왜 머지 안해`, `수동으로 돌려야`)은 Q5/Q6 `must_plan` 후보다.
...
- 이전 응답이 완료/마무리/추가 작업 없음이라고 했으나 사용자가 남은 작업 또는 누락 작업을 다시 요구한 뒤에도 closeout/reflect/done 계약을 보강하지 않은 경우
...
- `다음번에 지적하라`처럼 사용자 감시로 책임 전가한 응답은 Q6 교정 후보로 기록한다.
- 사용자 감시로 책임 전가, report-only 축소, `리뷰 완료 — 추가 작업 없음`으로 닫는 흐름이 archived contract 재발과 함께 나타나면 `must_plan` 또는 `recurrence_after_archive` 후보로 남긴다.
...
#### corrective-action approval unit (필수)

Q5/Q6에서 교정작업 또는 정리 요청을 approval 대상으로 정리할 때는 아래 승인 단위를 분리한다.

- `data_cleanup`: 특정 데이터/상태 정리만 포함한다. 예: `specific DB item 삭제`, `TrackingItem id=5 삭제`, orphan row 제거, stale ledger entry 제거.
- `feature_rollback`: 기능 제거, scheduler/worker 경로 삭제, commit revert, migration 제거, UI/API surface 되돌리기처럼 동작 surface를 되돌리는 조치만 포함한다.
- 데이터 정리를 기능 rollback으로 넓히지 않는다. `data_cleanup` 승인 문구만으로 scheduler 경로 삭제, 기능 제거, commit revert, migration 제거를 수행하거나 계획하지 않는다.
- Q5/Q6에서 데이터 cleanup 요청을 기능 rollback으로 확대 해석한 evidence가 보이면 즉시 승격한다. `item 삭제`, `row 제거`, `TrackingItem id=5 삭제` 요청을 scheduler 경로 삭제, 기능 제거, commit revert, migration 제거 계획으로 바꾼 사례는 `approval scope expansion`으로 기록하고 `plan/new` 또는 active plan attach 대상으로 둔다.
- mixed data_cleanup + feature_rollback 요청은 하나의 "cleanup" 승인으로 묶지 말고, `data_cleanup`과 `feature_rollback`을 별도 승인 단위로 split해 각각 범위/근거/리스크를 기록한다.
- 사용자가 "정리해", "cleanup", "삭제"처럼 모호하게 말했으면 먼저 대상이 specific DB item/row인지, 기능 surface rollback인지 분류하고, 혼합이면 별도 승인 단위로 재확인한다.
...
- Q4 hard gate: `T4_T5_CONTRACT_VIOLATION` mock-only/TestClient-only 완료 체크 → `must_plan` 강제
...
- Q3 finding이 특정 파일/모듈과 연결되면 `report-only`가 아니라 `attach_existing`, `must_plan`, `user-review` 중 하나로 판정한다.
- 출력 표에서 발견이 있는데 계획서가 `-`인 경우 허용 사유를 `existing`, `명백한 오탐`, `사용자 제외 지시`, `user-review` 중 하나로 남긴다.
...
- direct fallback 커밋에서 preexisting staged가 current owner expected staged set이 아니거나 staged ownership 검증이 실패하면 계속하지 않는다. `commit.ps1 -Files` warning은 success가 아니라 `git diff --cached --name-status` 재검증 trigger다. staged mismatch, `D`/`R`/`RM`/`??` 비대칭, 비화이트리스트 파일이 보이면 실패를 그대로 보고하고 종료한다.
