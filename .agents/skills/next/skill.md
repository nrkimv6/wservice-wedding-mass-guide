...
plans 워크트리가 dirty이면 `Test-PlansDirty`로 감지하고, 현재 세션 수정분과 기존 잔존 변경을 분리해야 한다고 먼저 경고한다. 에이전트 컨텍스트(`$env:CLAUDE_RUNNER_CONTEXT` 또는 auto-impl/auto-done/plan-runner/dev-runner)면 경고 후 계속 진행하고, 사람 세션이면 경고 + 수동 복구 명령만 제시한다. `git add -A`는 사용하지 않는다.

## 작업 소스 스캔 순서

1. **plans/TODO In Progress** - 이미 진행 중인 작업 (최우선)
2. **TODO Pending + Plan 문서** - 동등한 우선순위로 모두 스캔
   - `.worktrees/plans/TODO.md`의 Pending 항목
   - AGENTS.md 문서 위치 규칙의 plan 경로 미완료 항목
   - `{project}/docs/plan/*.md`의 미완료 항목 (프로젝트별 계획)
   - **분리된 plan 탐색**: plan 파일에 `> **실행 TODO:**` 링크가 있으면, 각 링크 대상 `_todo-N.md`에서 미완료 `[ ]`를 검색. 모든 `_todo-N.md`가 완료면 해당 plan 스킵
   - 모든 항목을 분석하여 **가장 중요하고 영향력 있는 작업** 선택
   - 고려 요소: 기능 중요도, 사용자 경험 개선, 기술 부채 해결, 의존성
3. **개선 아이디어** - AGENTS.md 문서 위치 규칙의 improvement 경로 (P0 → P1 순)
...
**plans/TODO.md 확인:**
- wtools docs commit root의 `.worktrees/plans/TODO.md` 파일 확인
- `## In Progress` 섹션에 항목 있으면 → 해당 작업 선택
- 없으면 `## Pending` 섹션 첫 번째 항목 선택
- repo root `{project}/TODO.md`, `{project}/docs/DONE.md`, `wtools/TODO.md`는 legacy/stub 또는 downstream mirror read-back 대상이며 `/next`의 기본 선택 source가 아니다.
...
**개선 아이디어 확인:**
- AGENTS.md 문서 위치 규칙의 improvement 경로 스캔
- `완료` 표시 없는 P0 항목 찾기
- 없으면 P1 항목 찾기
...
2. **파일 수정**: 선택한 항목의 `[ ]` → `[→$workerId]`로 변경
   - plan 파일: AGENTS.md 문서 위치 규칙의 plan 경로
   - TODO 파일: `.worktrees/plans/TODO.md`
...
3. **완료 처리**
   - **워크트리 사용 시** (plan 헤더에 `> branch:` 있음): `/merge-test`(같은 parent 배치 가능) → `/done` 순서로 호출
   - **워크트리 미사용 시**: 바로 `/done` 호출
   - done 스킬이 처리: plan 체크, `.worktrees/plans/TODO.md`→`.worktrees/plans/docs/DONE.md`, 아카이브, 검증, 커밋
...
다음 중 하나를 해보세요:
1. AGENTS.md 문서 위치 규칙의 plan 경로에 새 계획 문서 작성
2. `.worktrees/plans/TODO.md`에 작업 추가
3. AGENTS.md 문서 위치 규칙의 improvement 경로 검토
```

## 0단계: plans/TODO.md 최신 여부 확인

**스캔 시작 전** `.worktrees/plans/TODO.md`의 "마지막 업데이트" 날짜를 확인합니다:

1. `.worktrees/plans/TODO.md`의 "마지막 업데이트" 날짜 읽기
2. 오늘 날짜와 비교
3. **당일이면** → 그대로 `.worktrees/plans/TODO.md`에서 스캔 (빠름)
4. **이전 날짜면** → AGENTS.md 문서 위치 규칙의 plan 경로를 스캔하여 `.worktrees/plans/TODO.md` 갱신 후 스캔

> plans/TODO.md가 최신이면 이 파일 하나만 읽으면 되므로 빠름.
> `/implement`, `/done`, `/plan` 실행 시 자동 갱신되므로 대부분 최신 상태.

## 스캔 범위 옵션

- `/next` → **.worktrees/plans/TODO.md**에서 In Progress → Pending 순서로 확인 (빠름, 기본)
...
> plans/TODO.md가 최신이면 파일 1개만 읽어서 즉시 실행하여 세션 완료율 향상.
