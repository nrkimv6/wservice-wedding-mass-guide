...
- **있으면**: wtools 내부 → AGENTS.md의 plan 경로에 공통 계획 저장 + `.worktrees/plans/TODO.md` ledger 등록 **실행**
- **없으면**: 외부 프로젝트 → AGENTS.md의 plan 경로에 저장 + 해당 프로젝트의 `_path-rules.md`/plans ledger 정책을 따른다
...
- cross-surface 검토 결과 표 컬럼은 최소 `surface`, `변경 유형(추가/수정/삭제)`, `상대 surface 반영`, `사유`를 포함한다. `상대 surface 반영` 값은 `반영|비반영|별도 계획|해당 없음` 중 하나로 기록한다.
