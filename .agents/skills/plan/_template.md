...
plan 본문 또는 대상 코드/diff에 detector seed가 있으면 이 섹션을 유지하고, seed가 0개인 CRUD/문서 plan이면 `N/A: 상태 머신 detector seed 없음`으로 닫는다.
...
| 축 | TC/evidence 계획 |
|----|------------------|
| `creator` | 중요한 상태를 처음 만드는 경로를 검증한다 |
| `preserver` | cleanup/retry/fallback 후에도 상태가 보존되는지 검증한다 |
| `overwrite-block` | 늦은 writer가 terminal/waiting 상태를 덮지 못하는지 검증한다 |
| `override` | 명시적 사용자 action만 guard를 우회하는지 검증한다 |
| `display` | API/UI/read-model이 stale metadata와 terminal state 충돌을 올바르게 표시하는지 검증한다 |
| `late-writer ordering` | 동시 또는 시간차 writer 순서 의존 결함을 검증한다 |

> 축이 해당 없으면 축 이름과 함께 `N/A: 사유`를 적는다. "관련 TC 존재" 또는 vocabulary grep 통과만으로는 완료 조건이 아니다.
...
DS. ☐ **Downstream Sync Phase** — T4/T5 선행 sync evidence gate
   ...
   - ☐ `{downstream canonical 또는 generated surface 파일}`: downstream file read-back 결과를 기록한다
...
**공통/복수 프로젝트:** plan은 AGENTS.md/CLAUDE.md 문서 위치 규칙의 plan 경로에, **TODO는 각 프로젝트별로 분리 생성**
...
> 진행률: 0/M (0%)
> 요약: {이 TODO 파일이 담당하는 Phase 범위 요약}

## 상태 머신 6축 TC matrix (조건부)

대상 코드/diff에 detector seed가 있으면 이 섹션을 유지하고, seed가 0개인 CRUD/문서 plan이면 `N/A: 상태 머신 detector seed 없음`으로 닫는다.
...
1. `.worktrees/plans/TODO.md` Pending에 참조 항목 추가:
   ```
   - ☐ **{제목}** — [plan]({상대경로}) (0/N, 0%)
   ```
2. 완료 이동 대상은 `.worktrees/plans/docs/DONE.md`인 점을 유지
3. SKILL.md 4단계(plans/TODO.md ledger 등록)로 복귀
4. SKILL.md 5.5단계(계획서 커밋)로 복귀
