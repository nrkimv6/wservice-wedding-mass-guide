...
6. **owner set 전원 `머지대기` 확인** — `> worktree-owner:` 값이 쉼표 포함(owner set)인 경우, 모든 owner plan의 상태가 `머지대기` 이상이어야 한다. 하나라도 미만이면 `OWNER_SET_NOT_READY` 중단.
7. **primary 먼저 머지 순서 강제** — 현재 plan이 attached owner(owner set 목록의 2번째 이후)인 경우, primary plan 상태가 `구현완료` 이상이어야 한다. 미만이면 `PRIMARY_MUST_MERGE_FIRST: primary={primary_plan_path}` 중단.
...
- owner set 미준비 → "OWNER_SET_NOT_READY: {미준비 plan 경로 목록}. 모든 owner plan이 머지대기 상태여야 합니다."
- primary 먼저 필요 → "PRIMARY_MUST_MERGE_FIRST: primary={primary_plan_path}. 먼저 primary plan을 /merge-test하세요."
```
...
- `main`이 아니면 → 아래 순서로 자동 전환 시도 (dirty 파일 유형: 문서/코드 구분 없음)
  0. 수동 세션에서는 `git stash push`를 실행하기 전에 사용자에게 stash 생성 여부를 확인한다. `plan-runner` / `dev-runner` 자동 컨텍스트는 예외다.
...
- `> worktree-owner:` 필드가 있으면 값을 **쉼표로 split + trim** 후 `parent_plan_path`가 포함되어 있는지 확인한다 (대소문자 무시, `\`/`/` 정규화). owner set은 쉼표 구분 목록이거나 단일 경로 둘 다 허용한다.
- 포함되어 있지 않으면 즉시 중단한다. (다른 부모 계획서 워크트리 사용 금지)
...
> **attach 모드 비포함**: 배치 수집은 `parent_plan_path` 단일 기준 유지. attached plan(owner set의 2번째 이후)은 배치에 자동 포함 안 됨. 각 plan이 각자 `/merge-test`를 호출한다 (순서: primary 먼저, attached 나중).
...
- `$serviceLockTargets`가 1개 이상이고 `$runningServices`도 있으면 **경고 후 사용자 확인**:
  - 로그 prefix: `MERGE_PRECHECK_WARN[service_lock]` `targets={service_run.py 경로} running={서비스명}`
  - `Read-Host "서비스 실행 중 머지를 진행하시겠습니까? (y/N)"` (기본값 No)
    - 거부 또는 Enter 시: 로그 prefix `MERGE_PRECHECK_ABORTED[service_lock]`, `git merge`는 시도하지 않는다.
    - 승인(`y`) 시: 머지를 계속 진행한다.
  - 현재 세션에서 `nssm stop`, `Stop-Service`, `taskkill` 우회 시도 금지
  - 관리자 PowerShell 명령은 **권장 대응으로만** 출력 (강제 중단 아님):
  ...
  - ⚠️ runtime 경계: `common/tools/plan-runner/core/merge_stage.py`의 service_lock 분기는 `approval_required` 별도 surface이며 이 SKILL과 별개다.
...
<!-- service_lock만 WARN+confirm으로 완화됨. 다른 precheck(untracked_overwrite 등)는 hard stop 유지.
     판정 범위 기준: 2026-04-21_fix-merge-test-service-lock-precheck-diff-scope archive 결과 유지. -->
...
- plan/todo에 `Phase DB-Direct`가 있으면 실행 근거 표에 `실행 SQL/명령`, `존재 확인 쿼리`, `live API 또는 runtime 결과` 3개 row를 함께 보존한다.
- 4단계에서 실행한 명령 목록과 결과를 보존하지 못하면 8단계 완료 안내에서 T4/T5 완료를 선언하지 않는다.
- `Phase DB-Direct`가 있는 plan에서 위 3종 evidence를 보존하지 못하면 DB-direct/live validation 완료도 선언하지 않는다.
...
foreach ($target in $merge_targets) {
  if (git status --porcelain) {
    ...
    Write-Host "  → unrelated dirty를 흡수할 수 있으므로 git add . 자동 cleanup은 금지합니다."
...
| DB-direct-sql | `{실행 SQL/명령}` | {완료/미실행/해당 없음/실패} | {로그} |
| DB-direct-exists | `{존재 확인 쿼리}` | {완료/미실행/해당 없음/실패} | {로그} |
| DB-direct-live | `{live API 또는 runtime 결과}` | {완료/미실행/해당 없음/실패} | {로그} |
...
DB-direct/live 완료 표기는 위 실행 근거 표에 실제 실행 명령과 결과가 있을 때만 사용한다.
...
`merge`, `broad pytest`, `collect-only`만 수행한 상태를 `DB-direct/live validation 완료`로 보고해도 안 된다.
