...
> **호출 컨텍스트**: 독립 워커(Codex/plan-runner). 머지 + T4/T5 결과는 plan 상태 전이 + artifact 출력 기준으로 구조화.
...
| 4 | `next owner step = /done` | **continue** | merge 완료 후 같은 턴에서 `/done` 실행 |
| 5 | `archive/read-back` 미완료 | **continue** | `/done` owner가 닫을 때까지 진행 |
| 6 | 위 조건 모두 해당 없음 | **final** | 완료 보고 허용 |

> `merge success`, `T4/T5 passed` 단독은 final 사유 불가 — row 4·5를 먼저 확인한다.
> `merge → T4/T5 → /done` 전체 체인이 row 6에 도달할 때만 final 허용.
...
3. **프론트엔드 검증 확인** (webapp-testing 스킬):
   ...
   frontend 검증 실패 시 → 머지 롤백(`git reset --merge HEAD~1`), plan 상태 `머지대기`로 롤백, 워크트리 보존 후 **이후 단계 중단**.
...
**🔴 `/done` 호출 직전 parent-child open gate:**
- merge-test가 `/done`을 호출하기 직전에 parent plan의 linked child plan 상태를 1회 점검한다.
- child plan이 `구현중`/`초안`/`검토완료` 등 active/incomplete 상태이면 parent를 `머지완료(자식대기)` 상태로 남기고 `/done` 호출을 보류한다.
- **`머지완료(자식대기)` 상태는 archive 대상이 아니다.** child가 모두 완료된 뒤 다시 `/done`을 호출해야 한다.
- 사용자 명시 detach approval evidence(plan 본문 또는 review-plan 결과에 승인 문장)가 있는 child는 이 gate에서 제외한다.
