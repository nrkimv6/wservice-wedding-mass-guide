...
### 1.9단계: 다중 프로젝트 _todo-N.md 인식

plan에 `> **실행 TODO:**` 링크가 있으면 각 `_todo-N.md`를 Read하여:
1. `> 대상 프로젝트:` 필드로 현재 처리 중인 todo의 프로젝트 식별
2. `> 테스트명령:` 필드가 있으면 T4/T5 실행 시 해당 명령 사용
3. `> 선행조건:` 필드가 있으면 선행 todo의 완료 여부 확인 — 미완료면 경고 후 중단

**다중 프로젝트 merge-test 실행 순서:**
- 각 `_todo-N.md`를 실행순서(N) 기준으로 처리
- child(낮은 N) 먼저 머지 + T4/T5 → parent(높은 N) 머지 + T4/T5
- child 머지 실패/테스트 실패 시 parent도 중단

### 2단계: 머지 실행

**cwd를 `> 대상 프로젝트:` 기반으로 결정** (워크트리 밖):
...
T4/T5가 없으면 → 5단계(정리)로 바로 이동.

### 4단계: T4/T5 탐지 및 실행

**T4/T5 탐지**: plan 문서에서 아래 패턴 확인:
- `### Phase T4` 또는 `T4:` 체크박스
- `### Phase T5` 또는 `T5:` 체크박스
...
6. 각 T4/T5 체크박스 `[ ]` → `[x]` 업데이트, Read로 반영 확인
...
모든 테스트 통과 (또는 T4/T5 없이 머지만 완료) 후:
