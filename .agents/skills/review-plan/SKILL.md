...
- **prose detector**: 체크박스 외 본문 prose(개요/배경/범위/기술적 고려사항)에서도 child repo local mirror direct edit 의도를 감지한다.
  - 차단 seed: `local mirror sync`, `monitor-page (.agents|.claude|.gemini)`, `직접 동기화`, `mirror stale 보정`, `원본과 맞춤`, `manual sync`
  - 허용 seed: `git pull`, `/pull-sync`, `sync commit 수신`, `downstream read-back`, `GitHub Actions sync workflow`
...
- downstream mirror surface를 wtools 원본과 맞추기 위한 정당화에는 사용 불가
- 허용 사례는 프로젝트 한정 runtime rule처럼 wtools 원본에 두면 다른 프로젝트가 잘못 적용받을 수 있는 예외로 제한하며, 사용자 명시 사유가 필요하다.
- 허용 시 downstream sync evidence와 owner 사유를 plan 기술적 고려사항에 함께 남긴다.
