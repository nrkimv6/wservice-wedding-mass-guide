...
- **prose detector**: 체크박스 외 본문 prose(개요/배경/범위/기술적 고려사항)에서도 child repo local mirror direct edit 의도를 감지한다.
  - 차단 seed: `local mirror sync`, `monitor-page (.agents|.claude|.gemini)`, `직접 동기화`, `mirror stale 보정`, `원본과 맞춤`, `manual sync`, `mirror sync merge exception`, `MERGE_HEAD + sync subject`, `mirror-only staged`
  - 허용 seed: `git pull --ff-only`, `/pull-sync` ff-only 수신, `remote fast-forward 수신`, `downstream read-back`, `GitHub Actions sync workflow`
  - 차단 seed 1개 이상이고 허용 seed가 0개이면 `옵션 1 — 이관` 또는 `옵션 3 — 차단`만 허용한다.
  - 허용 seed가 함께 있으면 advisory evidence로 낮추고, 직접 edit/commit 의도가 아닌 pull 수신 검증인지 근거를 기록한다.
...
**옵션 2 — receiver sync 검증:**
- child repo mirror 파일을 수정하지 않는 `git pull --ff-only` 수신 검증 또는 downstream read-back plan만 허용한다.
- fence audit 결과: `허용: remote fast-forward/read-back only` 표시
- `MERGE_HEAD + sync subject + mirror-only staged` 같은 root local merge 예외 생성은 허용하지 않는다.
- receiver sync 검증 plan은 `git fetch origin` + `git rev-list --left-right --count HEAD...origin/main` matrix를 먼저 기록해야 한다. `ahead-only`, `behind-only`, `diverged`, `equal` 중 하나를 분류하고, `diverged`(`left>0 && right>0`)이면 push-first 금지 및 `DOWNSTREAM_DIVERGED_PUSH_BLOCKED`를 요구한다.
- 프로젝트 한정 runtime rule이 필요하면 child repo mirror가 아니라 wtools 원본 또는 별도 project-owned non-mirror surface로 이관한다.
- 허용 시 downstream sync evidence와 ff-only/read-back 근거를 plan 기술적 고려사항에 함께 남긴다.
...
1. 각 `/expand-todo` 호출 후 `git status --porcelain`으로 잔여 변경 확인
   - 비어있으면 커밋 **스킵**
   - 변경이 있으면 아래 절차로 커밋
2. 화이트리스트 파일만 개별 스테이징: `{plan경로}/*.md`, `TODO.md`
   - `git add .` / `git add -A` 사용 금지
3. `git diff --cached --name-only` 결과가 이번 실행의 입력 plan/TODO 화이트리스트와 정확히 일치하는지 검증
4. `git diff --cached --name-status` 또는 `git status --porcelain`에 `D`, `R`, `RM`, `??`가 섞여 있거나 비화이트리스트 경로가 남아 있으면 **즉시 중단**
   - `git reset HEAD -- <path>`로 일부만 걷어내고 계속 진행하지 않는다.
   - 복구는 `전체 unstage -> 원하는 파일만 다시 add` 패턴으로 처음부터 다시 스테이징한다.
5. 커밋 메시지: `docs: review — {계획서 제목 요약}`
