...
1.2. **워크트리 준비 (수동 세션 main 오염 방지)**
   ...
   **B. 잔여 워크트리/브랜치 감지:**
   ...
   - 잔여분이 있으면 소유권을 먼저 판정한다:
     - `docs/plan/**/*.md`에서 동일 `> branch:`/`> worktree:`를 검색
   ...
   **C. plan 헤더에서 `> branch:` 및 `> worktree:` 필드 확인:**
   ...
   - **필드가 있으면 (크래시 복구):**
     1. 소유권 검증:
        ...
        - 레거시 파일처럼 `> worktree-owner:`가 없으면 `docs/plan/**/*.md`에서 동일 `branch/worktree` 사용 파일을 검색해 부모를 역추적한다.
...
1.3. **main 기존 수정사항 무시 모드 (사용자 명시 지시 시)**
   ...
   - 루트(main worktree)의 기존 dirty 파일은 충돌/중단 사유로 취급하지 않는다.
